/*
 * ESP32-S3-EYE WiFi Time Reporter - HTTP Server
 * 
 * 功能：
 * 1. 连接WiFi
 * 2. 通过SNTP获取NTP实时时间
 * 3. 提供HTTP服务器，手机浏览器访问查看时间
 */
...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    ...
    static const char html_prefix[] = 
        ...
        "<body>"
        "<div class=\"container\">"
        "<div class=\"title\"><span class=\"status\"></span>ESP32 时间上报</div>"
        "<div class=\"time\" id=\"time\">--:--:--</div>"
        "<div class=\"date\" id=\"date\">----/--/--</div>"
        "<div class=\"weekday\" id=\"weekday\">--</div>"
        "<div class=\"timezone\">时区: Asia/Shanghai (CST-8)</div>"
        "<div class=\"ip\">IP: ";
    ...
    /* 构建完整HTML */
    char *response = malloc(strlen(html_prefix) + strlen(ip_str) + strlen(html_suffix) + 1);
    if (response == NULL) {
        ESP_LOGE(HTTP_TAG, "Failed to allocate memory for HTML response");
        httpd_resp_send_500(req);
        return ESP_FAIL;
    }
    
    strcpy(response, html_prefix);
    strcat(response, ip_str);
    strcat(response, html_suffix);
    ...
    ESP_LOGI(HTTP_TAG, "Time page accessed");
    return ESP_OK;
}
