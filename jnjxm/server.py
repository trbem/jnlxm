...
async def register(request: Request):
    ...
    if len(username) < 2 or len(username) > 20:
        raise HTTPException(status_code=400, detail="用户名长度应在2-20个字符之间")
    ...
    # 创建新用户
    user_config = {
        "name": nickname or "知心伙伴",
        "system_prompt": config["system_prompt"],
        "model": config["model"],
        "max_context_rounds": 10,
        "quick_replies": DEFAULT_QUICK_REPLIES.copy()
    }
    ...
    return {
        ...
        "config": {
            ...
            "quick_replies": user_config["quick_replies"]
        }
    }
